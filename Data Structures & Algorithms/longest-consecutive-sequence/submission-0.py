class Solution:
    def longestConsecutive(self, nums: List[int]) -> int:
        ...
        for n in nums:
            prev_neighbor: int = n - 1
            if prev_neighbor not in set_nums:
                seq: int = 0
                # We are at a root, build the sequence until we don't have any more children in the set
                node: int = n
                while node in set_nums:
                    seq += 1
                    node += 1
