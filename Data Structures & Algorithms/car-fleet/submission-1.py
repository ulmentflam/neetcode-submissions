class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        pairs.sort(reverse=True) # O(n log(n))
        stack: List[float] = [] # The intuition here is the stack holds the number of fleets, space O(n)
        # We are iterating in decending order from highest to lowest.
        for p, s in pairs: # O(n)
